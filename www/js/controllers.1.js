...
$scope.sendUrl = function(jsParam){
  ...
  var interval = $interval(function () {
      ...
      if (queryFormDetail_RESULT) {
        ...
        if(tools.isNull(pendingDetailList.data)){
          ...
          if(tools.isNull($scope.doneWork.app_node) == false){
            ...
            if((($scope.editableBusinessForm_button == 'false')|| ($scope.doneWork.app_node!='handle1')) == false){
              $scope.is_meetresolution_Class ="item116 listFont1 pos_rel";
            }
            if((($scope.editableBusinessForm_button == 'false')|| ($scope.doneWork.app_node!='handle1')) == false){
        ...
        if(tools.isNull(queryRouteListParam.data) == false){
...
$scope.deleteEmptyProperty = function(obj){
  ...
  $scope.Signin = function(){
    ...
    // //检验实际付款日期
    else if( $scope.pendingWorkId == "payment" && tools.isNull($scope.doneWork.actualPaymentDate)){
      var alertPopup = $ionicPopup.alert({
        title: '请选择实际付款日期!'
      });
      flag = false;
    ...
    if(flag){
      ...
      var saveFormParam ={
        ...
        actual_pay_date:$scope.doneWork.actualPaymentDate+"",
  ...
  $scope.Signout = function(){  
    ...
    if(tools.isNull($scope.in.isReject)){
      ...
      // //检验实际付款日期
      else if(  $scope.doneWork.app_node == "3" && $scope.pendingWorkId == "payment" && tools.isNull($scope.doneWork.actualPaymentDate)){
        var alertPopup = $ionicPopup.alert({
          title: '请选择实际付款日期!'
        });
        flag = false;
  ...
  var datePickerCallbacke3 = function (val) {
    ...
    if (typeof (val) === 'undefined') {
      ...
      $scope.doneWork.actualPaymentDate = formatDateTime(val);//更新日期。
    }
  };
  //转让基准日选择后的回调函数
...
.controller('loginCtrl', function($scope,tools,$state,$ionicHistory,$rootScope,$stateParams,$ionicSlideBoxDelegate) {
  $scope.items=[
                {prizeList:[
